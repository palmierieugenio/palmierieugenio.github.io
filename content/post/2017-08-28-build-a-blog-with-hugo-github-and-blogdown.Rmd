...
####1. Create [a Github Account](https://github.com/) with your USERNAME[^2] as account name:
...
`USERNAME.github.io`, In my case `palmierieugenio.github.io`, it will be your working directory.

####4. Run in the Git CMD terminal
You should have it istalled with the Git installation:
...
```
####5. Open RStudio and select the Menu > File > New Project:
...
It is the configuration of the blog[^5].
...
blogdown::serve_site()
```
Now you should see a local version of your blog in RStudio. You should be able to display it on your browser with the button "Show in new window".[^6]
...
To create a post locally, you have to create a .md o .Rmd file in the `path\USERNAME.github.io\content\post` folder. A fast way to do it is with the button `New Post` in the Addins menu of RStudio, but it is possible to create a file manually or just as a copy of another article (there are post of example at the creation of the site). You can see more info in the [Academic template Demo](https://sourcethemes.com/academic/).
...
We DON'T need necessarily to use these commands after `cd /Git/[USERNAME].github.io`, because we have already used them the first time:
```{bash eval=FALSE}
git init # initiates repo in the directory
git remote add origin https://github.com/[USERNAME]/[USERNAME].github.io # connects git local repo to remote Github repo
```
...
```{r eval=FALSE}
disqusShortname = "esempio_disqus_shortname"
```

To make the changes effective, you have to upload them on GitHub with the Git CMD terminal:
```{bash eval=FALSE}
cd /Git/[USERNAME].github.io # your path to the repo may be different
git add . # indexes all files that wil be added to the local repo
...
[^6]: There should be a button,`Serve Site`, in RStudio for the function `blogdown::serve_site()`, in the Addins menu.
